...
def _default_gstin(gstin: str | None) -> str:
    """Fall back to configured sandbox GSTIN or 'NA'."""
    return gstin or getattr(settings, "GST_SANDBOX_GSTIN", None) or "NA"
...
async def get_gstr3b_json(
    ...
    gstin: str | None = Query(None, description="GSTIN to use in JSON"),
    db: AsyncSession = Depends(get_db),
) -> dict[str, Any]:
    """
    Debug: Build GSTR-3B JSON from DB invoices for a user + period.
    """
    ...
    use_gstin = _default_gstin(gstin)
...
async def get_gstr1_json(
    ...
    """
    Debug: Build GSTR-1 JSON from DB invoices for a user + period.
    """
    period_start = _parse_period(period)
    last_day = monthrange(period_start.year, period_start.month)[1]
    period_end = date(period_start.year, period_start.month, last_day)
...
async def list_invoices(
    ...
    """
    Debug: List all invoices for a user + period (see what OCR + parser produced).
    """
    period_start = _parse_period(period)
    last_day = monthrange(period_start.year, period_start.month)[1]
    period_end = date(period_start.year, period_start.month, last_day)
        ...
        items.append(
            {
                ...
                "taxable_value": float(inv.taxable_value) if inv.taxable_value is not None else None,
                ...
                "cgst_amount": float(inv.cgst_amount) if inv.cgst_amount is not None else None,
                "sgst_amount": float(inv.sgst_amount) if inv.sgst_amount is not None else None,
                "igst_amount": float(inv.igst_amount) if inv.igst_amount is not None else None,
            }
        )
